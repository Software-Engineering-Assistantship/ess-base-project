...
const feature = loadFeature('../features/metodos_de_pagamento.feature');
...
defineFeature(feature, test => {
  ...
  test('Cadastro de novo cartão de mesmo card_number, mas types diferentes', ({ given, when, then, and }) => {
    ...
    then(/^o cadastro do cartão com o card_number "(.*)" e type "(.*)" no banco de dados é realizado com sucesso$/, async(card_number, type) =>{
      const cardExists = await prismaTestClient.cardPayment.findMany();

      expect(cardExists[0].card_number).toBe(card_number);
      expect(cardExists[1].card_number).toBe(card_number);
      expect(cardExists[0].type).toBe('credit');
      expect(cardExists[1].type).toBe(type);
    });
