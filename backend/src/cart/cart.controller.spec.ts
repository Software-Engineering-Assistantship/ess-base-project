...
describe('CartController', () => {
  ...
  beforeAll(async () => {
    const mockCartService = {
      ...
      updateCartItem: async (id: string, itemId: string, updatedItem: UpdateMenuItemSchema) => Promise.resolve(),
      deleteCartItem: async (id: string, itemId: string) => Promise.resolve(),
      addItemToCart: async (id: string, newItem: CreateMenuItemSchema) => Promise.resolve(),
    };
  ...
  describe('getCartById', () => {
    it('should return a cart by id', async () => {
      ...
      const spyGetCartById = jest.spyOn(cartService, 'getCartById').mockResolvedValue(expectedCart);
    ...
    it('should throw NotFoundException if cart is not found', async () => {
      ...
      jest.spyOn(cartService, 'getCartById').mockRejectedValue(new NotFoundException());

      await expect(controller.getCartById(cartId)).rejects.toThrowError(NotFoundException);
    });
  });
  ...
  describe('addItemToCart', () => {
    it('should add item to cart', async () => {
      ...
      const updatedCart = await controller.addItemToCart(cartId, mockCreateMenuItem);
