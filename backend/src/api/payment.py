...
# classe dos cartões a serem cadastrados
...
# lista de cartões e seu valor inicial
cartoes : list[credit_card] = []

@router.get(
    "/{item_id}", 
    response_model=HttpResponseModel,
    status_code=status.HTTP_200_OK,
    description="Acessar um cartao de credito",
    tags=["payment"],
    responses={
        status.HTTP_200_OK: {
...
@router.post('/')
def adding_card(cartao : credit_card):
